-- Phase 10C review fix — surface milestone-submission progress in the 360 missions section.
-- Spec §5 names mission_participants(+milestone submissions) as a 360 source; the initial
-- aggregator omitted submissions. Re-create admin_creator_detail (never edit the shipped
-- 20260629120000 migration) adding per-participant submission counts: total / approved /
-- pending (pending = status in submitted|pending|revision_requested). Everything else is
-- unchanged. Counts come from public.mission_milestone_submissions (linked via
