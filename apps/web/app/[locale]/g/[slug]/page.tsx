...
export async function generateMetadata({
  ...
  const creator = getCreator(guide.creatorHandle)
  return {
    title: `${guide.title} | KINNSO`,
    description: `${guide.city} guide by ${creator?.name ?? `@${guide.creatorHandle}`}. ${guide.saves.toLocaleString()} saves.`,
  }
}
    ...
    <article className="k-container py-8 md:py-12">
      ...
      <section className="mt-6 grid gap-5 md:grid-cols-[1fr_320px]">
        <div className="rounded-lg bg-white p-6">
          ...
          <p className="mt-2 text-sm text-kinnso-muted">{guide.city}</p>
        ...
        <aside className="rounded-lg bg-kinnso-cream2 p-6">
          ...
          <div className="mt-3">
            <div className="text-lg font-black text-kinnso-ink">{creator?.name ?? guide.creatorHandle}</div>
            <Link href={`/${locale}/c/${guide.creatorHandle}`} className="k-mono mt-1 inline-flex text-sm text-kinnso-orange hover:text-kinnso-orangeDark">
              @{guide.creatorHandle}
            </Link>
          </div>
        </aside>
